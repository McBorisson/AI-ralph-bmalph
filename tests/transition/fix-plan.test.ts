...
describe("fix-plan", () => {
  describe("generateFixPlan", () => {
    ...
    it("replaces every dot when building malformed story anchors", () => {
      const stories = [makeStory({ id: "1.2.3" })];
      const plan = generateFixPlan(stories);

      expect(plan).toContain("Spec: specs/planning-artifacts/stories.md#story-1-2-3");
    });
...
- [x] Story 2.1: Dashboard`;
    ...
    it("parses malformed numeric-dot IDs generated by forced runs", () => {
      const content = `- [x] Story 1..2: Broken identifier
- [ ] Story 1.2.3: Extra segment`;

      const items = parseFixPlan(content);

      expect(items).toEqual([
        { id: "1..2", completed: true, title: "Broken identifier" },
        { id: "1.2.3", completed: false, title: "Extra segment" },
      ]);
    });
  });
  ...
  describe("mergeFixPlanProgress", () => {
    ...
    it("preserves completion for malformed IDs from a forced run", () => {
      const plan = "- [ ] Story 1..2: Broken identifier\n- [ ] Story 1.1: Login";
      const completed = new Set(["1..2"]);

      const merged = mergeFixPlanProgress(plan, completed);

      expect(merged).toContain("- [x] Story 1..2: Broken identifier");
      expect(merged).toContain("- [ ] Story 1.1: Login");
    });
  });
